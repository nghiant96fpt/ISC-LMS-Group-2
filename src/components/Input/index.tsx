...
const Input = forwardRef<HTMLInputElement, inputProps>(
  ({ placeholder, outline, disabled, icon, size, style, className, type, onChange, onKeyDown, onPaste }, ref) => {
    return (
      <div
      ...
      >
        {icon && (
          <div className={`absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none ${outline ? 'text-orange-text' : 'text-slate-400'}`}>
            {icon}
          </div>
        )}
        <input
