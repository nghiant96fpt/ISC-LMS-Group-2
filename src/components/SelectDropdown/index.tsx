    ...
    return (
        <div className={`relative w-full max-w-full ${className}`}>
            ...
            >
                ...
                <IconChevronDown
                    className={`text-base flex-shrink-0 transition-transform duration-200 ${open ? "rotate-180" : "rotate-0"}`}
                />
            ...
            {/* Dropdown Options */}
            {open && (
                <div className="absolute top-full left-0 w-full mt-2 max-h-[200px] z-50 bg-white border border-gray-400/10 rounded-lg shadow-lg overflow-y-auto space-y-2
                ...
                animate-fadeIn`}>">
                    {options.map((option) => (
                        <div
                            ...
                            className={`flex py-2 px-3 cursor-pointer transition-all duration-150 ${option === value ? "bg-orange-text text-white" : "hover:bg-gray-100"
                                }`}
                        >
                            <p className="text-sm w-full flex items-center mt-3">{option}</p>
                        </div>
                    ))}
                </div>
            )}
