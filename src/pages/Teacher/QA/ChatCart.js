const ChatCart = ({ chat, index, handleClick, cartIndexChoose }) => {
  return (
    <>
      <div
        onClick={handleClick}
        key={index}
        className={`flex items-center ${index === cartIndexChoose ? 'bg-gray-100' : ''} p-3 rounded-xl cursor-pointer transition ${
          chat.selected ? '' : 'hover:bg-gray-100'
        }`}
      >
        {/* Ảnh đại diện */}
        <div className="relative">
          <img src={chat.avatar} alt={chat.name} className="w-12 h-12 rounded-full object-cover" />
          {chat.active && <span className="absolute top-0 right-0 w-3 h-3 bg-orange-500 rounded-full"></span>}
        </div>
