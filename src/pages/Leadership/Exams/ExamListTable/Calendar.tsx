...
const Calendar = ({ setSelectedDate }: CalendarProps) => {
    ...
    const getColorByExam = (examList: Exam[]) => {
        ...
        if (exam.duration === "15 phút") return "bg-yellow-400";
        if (exam.duration === "45 phút") return "bg-blue-500";
        ...
        <div className="w-full max-w-screen-lg mx-auto bg-white-100 p-2 sm:p-4 rounded-2xl shadow-lg">
            <div className="bg-white rounded-lg  overflow-hidden">
                <div className="bg-black text-white flex flex-wrap items-center px-2 sm:px-4 py-2 sm:py-3 rounded-t-lg text-xs sm:text-base">
                    ...
                    <select className="mx-1 sm:mx-2 bg-black text-white text-xs sm:text-sm" value={currentDate.getMonth()} onChange={(e) => handleMonthChange(Number(e.target.value))}>
                        {Array.from({ length: 12 }, (_, i) => (
                            <option key={i} value={i}>Tháng {i + 1}</option>
                        ))}
                    </select>
                    <select className="mx-1 sm:mx-2 bg-black text-white text-xs sm:text-sm" value={currentDate.getFullYear()} onChange={(e) => handleYearChange(Number(e.target.value))}>
                    ...
                    <button className="ml-2 sm:ml-4 bg-green-500 text-white px-2 sm:px-4 py-1 sm:py-2 rounded" onClick={goToToday}>Hôm nay</button>
                </div>
                {renderWeekDays()}
                {renderDays()}
            </div>
