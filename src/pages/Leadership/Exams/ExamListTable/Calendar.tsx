    ...
    return (
      <div className="grid grid-cols-7 border-l border-orange-500 bg-white rounded-b-lg text-xs sm:text-sm">
        {Array.from({ length: daysInMonth }, (_, i) => {
          const day = i + 1;
          const formattedDate = `${currentDate.getFullYear()}-${(currentDate.getMonth() + 1).toString().padStart(2, '0')}-${day
            .toString()
            .padStart(2, '0')}`;
          ...
          return (
            ...
            >
              <p
                className={`text-gray-600 text-center w-6 h-6 sm:w-8 sm:h-8 flex items-center justify-center rounded-full ${
      ...
      <div className="rounded-lg overflow-hidden">
        <div className="bg-black text-white flex flex-wrap items-center px-2 sm:px-4 py-2 sm:py-3 rounded-t-lg text-xs sm:text-base">
          ...
          <div className="ml-auto flex gap-1 sm:gap-2">
            <button
              className={`px-2 sm:px-4 py-1 sm:py-2 rounded ${viewMode === 'month' ? 'bg-blue-500 text-white' : 'bg-gray-200'}`}
              onClick={() => setViewMode('month')}
            >
              Tháng
            </button>
            <button
              className={`px-2 sm:px-4 py-1 sm:py-2 rounded ${viewMode === 'week' ? 'bg-blue-500 text-white' : 'bg-gray-200'}`}
