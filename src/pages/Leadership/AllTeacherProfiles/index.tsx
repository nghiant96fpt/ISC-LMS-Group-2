...
const AllTeacherProfiles: React.FC = () => {
  const [urls, setUrls] = useState([{ link: "", linkName: "Hồ sơ giảng viên" }, { link: "", linkName: "Tất cả hồ sơ" }])
  ...
  const fetchData = useCallback(async (page: number, pageSize: number = itemsPerPage, searchQuery: string = searchTerm) => {
    setLoading(true)
  ...
  const handleConfirmDeleteAll = useCallback(async () => {
    ...
    try {
      ...
      const deleteResults = await Promise.allSettled(
        selectedTeachers.map((id) => deleteTeacher(id))
      );
    ...
    <div className="p-3">
      ...
      <div className="flex justify-between items-center mb-4">
        ...
        <div className="space-x-2 flex justify-between">
          ...
          <Button className="outline-primary" onClick={handleExportExcel}>Xuất file</Button>
          <Button className="primary" onClick={goAddTeacher} >+ Thêm mới</Button>
        </div>
      </div>
          ...
          <table className="min-w-full   border-collapse overflow-hidden rounded-t-lg">
            ...
            <tbody className="whitespace-nowrap">
              {teachers?.length > 0 ? (
                teachers.map((teacher, index) => (
                  <tr key={index} className={`border-b ${index % 2 === 1 ? 'bg-gray-100' : ''}`}>
                    ...
                    <td className="py-3 px-4 text-black-text ">
                      {dataSubject.find(sub => sub.id === teacher.subjectId)?.name || 'Chưa rõ'}
                    </td>
                ...
                <tr className="text-center "><td className="bg-[#F0F3F6]" colSpan={9}>{message}</td></tr>
              )}
            </tbody>
          </table>
