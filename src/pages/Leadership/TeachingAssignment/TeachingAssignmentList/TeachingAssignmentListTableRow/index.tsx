...
const TeachingAssignmentListTableRow: React.FC<TeachingAssignmentListTableRowProps> = ({
    item, index, onDelete, onEdit
}) => {
    return (
        <TableRow key={item.id} className={index % 2 === 0 ? 'bg-[#F0F3F6] border-[#F0F3F6]' : ''}>
            ...
            <TableCell className="px-4 py-3 text-start">
                <div className="flex gap-2">
                    <button >
                        <img src={IconImages.iconClipboardList} alt="Teaching Assignment" className="w-4 md:w-5 lg:w-6" />
                    </button>
                </div>
            </TableCell>
