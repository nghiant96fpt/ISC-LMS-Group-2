...
interface AllStudentProfilesHeaderProps {
  onTabChange: (tab: string) => void;
}

const AllStudentProfilesHeader: React.FC<AllStudentProfilesHeaderProps> = ({ onTabChange }) => {
      ...
      <div className="flex justify-between items-center mb-6 mr-[30px]">
        <div className="flex gap-4">
          ...
          <div className="flex gap-3 button-group">
            {buttons.map((btn) => (
              <button key={btn.value} className={`btn ${activeButton === btn.value ? 'active' : ''}`} onClick={() => handleTabChange(btn.value)}>
                {btn.label}
              </button>
            ))}
          </div>
