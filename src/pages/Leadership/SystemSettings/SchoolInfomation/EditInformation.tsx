...
const EditSchoolInFo: React.FC = () => {
  ...
  useEffect(() => {
    axios.get('https://provinces.open-api.vn/api/?depth=1').then((res) => {
      ...
      console.log(res.data.map((p: any) => ({ value: p.code, label: p.name })));
    });
  }, []);

  useEffect(() => {
    if (selectedCity) {
      axios.get(`https://provinces.open-api.vn/api/p/${selectedCity}?depth=2`).then((res) => {
  ...
  const handleImageCSChange = (index: number, event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      reader.onload = (e) => {
        ...
        setSchoolData((prevData) => prevData.map((item, i) => (i === index ? { ...item, img: newImage } : item)));
      };
              ...
              <div className="col-span-2 grid grid-cols-2 gap-4 text-sm">
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <div className="flex justify-between">
                    ...
                    <input {...register('principalPhone')} className="border border-gray-300 p-2 rounded-[8px] w-2/3" placeholder="Số hiệu trưởng" />
                  </div>
                </div>
              </div>
            </div>
