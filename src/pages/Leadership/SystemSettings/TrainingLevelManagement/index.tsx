...
import { fetchTrainingLevels } from './../../../../redux/reducers/trainingLevelManagement';
...
const TrainingLevelManagement = () => {
  ...
  const totalPages = Math.ceil((TrainingLevelManagement?.length || 0) / itemsPerPage);
            ...
            <div className="overflow-x-auto border border-gray-400/10 rounded-lg max-h-[340px] overflow-y-auto">
              <Table>
                ...
                <TableBody className="divide-y divide-gray-300 bg-white">
                  {TrainingLevelManagement?.slice(startIndex, startIndex + itemsPerPage).map((item, index) => (
                    <ListOfTrainingLevelManagementListTableRow key={index} item={item} index={startIndex + index} />
                  ))}
                </TableBody>
              </Table>
            </div>
